    ...
    if repo_name == "ha-openwrt":
        ...
        try:
            req = urllib.request.Request(
                "https://api.github.com/repos/Hypfer/Valetudo/releases/latest", headers=headers
            )
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode("utf-8"))
        ...
        try:
            req = urllib.request.Request(
                "https://api.github.com/repos/Atmosphere-NX/Atmosphere/releases/latest", headers=headers
            )
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode("utf-8"))
                return data["tag_name"].lstrip("v")
...
def clean_and_update_template(file_path, integration_version, ha_version, repo_name):
    ...
    for i in range(2, len(blocks), 2):
        ...
        if field_id in ("integration_version", "version"):
            ...
            new_block = re.sub(r'placeholder:\s*(["\']?)(e\.g\.\s*)?[^\n"\']+\1', repl_ver, block_content)
        ...
        # 3. Service versions
            ...
            or (field_id == "atmosphere_version" and repo_name == "ha-NintendoSwitchCFW")
        ):
    ...
    for i, line in enumerate(lines):
        ...
        if "description:" in line:
            ...
            if (
                any(k in desc_lower for k in ["domain", "host", "ip address", "url", "instance", "address"])
